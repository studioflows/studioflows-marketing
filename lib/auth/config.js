export const PRODUCT_DESTINATIONS = {
  consulting: "https://consulting.studioflows.co/admin",
  vessa: "https://vessa.studioflows.co/vessa",
...
export function validateRelativeNext(nextValue) {
  ...
  if (nextValue.startsWith("//")) return null;
  const pathOnly = nextValue.split("?")[0];
  const allowed = new Set(["/apps", "/login", "/signup"]);
...
export function buildAuthCompleteUrl(context) {
  const completeUrl = new URL("/auth/complete", AUTH_ORIGIN);
  const query = authContextToQuery(context);
  if (!query) return completeUrl;
...
export function mergeAuthContext(primary, fallback) {
  return {
    product: primary?.product || fallback?.product || null,
    intent: primary?.intent || fallback?.intent || "login",
    returnTo: primary?.returnTo || fallback?.returnTo || null,
    next: primary?.next || fallback?.next || null,
  };
}
